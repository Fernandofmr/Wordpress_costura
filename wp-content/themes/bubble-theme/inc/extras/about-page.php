 ...
 *
 * Class Bubble_Extras_About_Page
 *
...
class Bubble_Extras_About_Page {
	...
	function about_page_notice() {
		...
		<div class="updated notice is-dismissible">
			<p>
				<?php echo esc_html( sprintf( __( 'Thanks for choosing %s!', 'bubble' ), $theme->get( 'Name' ) ) ); ?>
				<?php
				printf(
					esc_html__( 'Welcome to our bubble world. Please, don´t pop it!.', 'bubble' ),
					'<a href="' . admin_url( 'themes.php?page=bubble-theme-about' ) . '">',
					'</a>',
			...
			<p>
				<a href="<?php echo esc_url( admin_url( 'themes.php?page=bubble-theme-about' ) ); ?>" class="button-primary">
					<?php echo esc_html( sprintf( __( 'Learn About %s', 'bubble' ), $theme->get( 'Name' ) ) ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	function add_theme_page( ) {
		...
		add_theme_page(
			sprintf( __( 'About %s', 'bubble' ), $theme_name ),
			sprintf( __( 'About %s', 'bubble' ), $theme_name ),
			'edit_theme_options',
			'bubble-theme-about',
	...
	function enqueue_scripts( $prefix ) {
		if ( $prefix !== 'appearance_page_bubble-theme-about' ) return;

		wp_enqueue_script(
			'bubble-about',
			get_template_directory_uri() . '/inc/extras/js/about.js',
			array( 'jquery' ),
			BUBBLE_THEME_VERSION
		);
			...
			case 'twitter' :
				$share_url = add_query_arg( array(
					'status' => urlencode(
						$theme->get( 'Name' ) .
						' - ' .
						__( 'Free #WordPress Theme from @BubbleThemes', 'bubble' ) . ' - ' .
						$theme->get( 'ThemeURI' )
					)
	...
	function display_about_page(){
		...
		$theme = wp_get_theme( get_template() );
		$about = apply_filters( 'bubble_about_page', array(
			'title' => sprintf( __( 'About %s', 'bubble' ), $theme->get( 'Name' ) ),
			'sections' => array(),
			'title_image' => false,
		...
		?>
		<div class="wrap" id="bubble-about-page">
				...
				<?php if ( ! empty( $about[ 'tour_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'tour_url' ] ) ?>" class="about-button-tour" target="_blank">
							<?php esc_html_e( 'Take a Tour', 'bubble' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if( !empty( $about[ 'newsletter_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'newsletter_url' ] ) ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Get Updates', 'bubble' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if ( ! empty( $about[ 'documentation_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'documentation_url' ] ) ?>" class="about-button-docs" target="_blank">
							...
							<?php esc_html_e( 'Documentation', 'bubble' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if ( ! empty( $about[ 'review_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'review_url' ] ) ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Write a Review', 'bubble' ) ?>
						</a>
					</li>
				<?php endif; ?>

				<?php if ( ! empty( $about[ 'premium_url' ] ) && ! defined( 'BUBBLE_IS_PREMIUM' ) ) : ?>
					<li class="about-highlight">
						<a href="<?php echo esc_url( $about[ 'premium_url' ] ) ?>" class="about-button-updates" target="_blank">
							<span class="dashicons dashicons-arrow-up-alt"></span>
							<?php esc_html_e( 'Upgrade to Premium', 'bubble' ) ?>
						</a>
					</li>
				<?php endif; ?>
						...
						<?php if ( empty( $about[ 'no_video' ] ) ) : ?>
							<div class="about-video-watch">
								<a href="<?php echo esc_url( $about[ 'video_url' ] ) ?>" target="_blank">
									<?php esc_html_e( 'Watch The Video', 'bubble' ) ?>
								</a>
							</div>
						<?php endif ?>
						...
						<?php if ( $theme->get( 'ThemeURI' ) ) : ?>
							<div class="about-share">
								<div class="about-share-title">
									<?php echo esc_html( sprintf( __( 'If you like %s, please share it!', 'bubble' ), $theme->get( 'Name' ) ) ) ?>
								</div>
			...
			<div class="about-bubble-logo">
				<p>
					<?php echo esc_html__( 'Proudly Created By', 'bubble' ); ?>
				</p>
				<a href="https://bubblethemes.com/" target="_blank">
					<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/inc/extras/css/images/bubble.png" />
				</a>
			</div>
		</div>
		<?php
...
// Setup the single.
Bubble_Extras_About_Page::single();
