...
if ( ! function_exists( 'bubble_author_box' ) ) :
/**
 * Display the post author biographical info on single posts.
 */
function bubble_author_box() { ?>
	<div class="author-box">
		...
		<div class="author-description">
			...
			<span class="author-posts">
				<a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>">
					<?php esc_html_e( 'View posts by ', 'bubble' );
					echo get_the_author(); ?>
				</a>
			</span>
...
if ( ! function_exists( 'bubble_breadcrumbs' ) ) :
/**
 * Displays breadcrumbs supported by Breadcrumb NavXT, Rank Math, and Yoast SEO.
 */
function bubble_breadcrumbs() {
	if ( bubble_page_setting( 'overlap' ) != 'disabled' ) return;
...
add_action( 'bubble_content_top', 'bubble_breadcrumbs' );

if ( ! function_exists( 'bubble_mini_cart' ) ) :
/**
 * Display the WooCommerce mini cart.
 */
function bubble_mini_cart() {
	if (
		class_exists( 'Woocommerce' ) &&
		apply_filters( 'bubble_display_mini_cart', ! ( is_cart() || is_checkout() ) )
	) :
		global $woocommerce;
		?>
		<ul class="shopping-cart">
			<li>
				<a class="shopping-cart-link" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View shopping cart', 'bubble' ); ?>">
					<span class="screen-reader-text"><?php esc_html_e( 'View shopping cart', 'bubble' ); ?></span>
					<?php bubble_display_icon( 'cart' ); ?>
					<span class="shopping-cart-text"><?php esc_html_e( 'View Cart', 'bubble' ); ?></span>
...
if ( ! function_exists( 'bubble_comment' ) ) :
/**
 * The callback function for wp_list_comments in comments.php.
 *
 * @link https://codex.wordpress.org/Function_Reference/wp_list_comments.
 */
function bubble_comment( $comment, $args, $depth ) {
	?>
			...
			<div class="comment-container">
				...
				<?php if ( ! $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation">
						<?php esc_html_e( 'Your comment is awaiting moderation.', 'bubble' ); ?>
					</p>
				<?php endif; ?>
					<?php comment_text() ?>
				</div>
...
if ( ! function_exists( 'bubble_footer_text' ) ) :
/**
 * Displays the footer text.
 */
function bubble_footer_text() {

	$text = get_theme_mod( 'footer_text', esc_html__( 'Copyright &copy; {year} {sitename}', 'bubble' ) );
	if ( empty( $text ) ) return;
	$text = str_replace(
		array( '{sitename}', '{year}' ),
		array( get_bloginfo( 'sitename' ), date_i18n( esc_html__( 'Y', 'bubble' ) ) ),
		'<span>' . $text . '</span>'
	);
 ...
 */
function bubble_get_featured_posts() {
	return apply_filters( 'bubble_get_featured_posts', array() );
}

/**
 * Check the Jetpack Featured Content.
 */
function bubble_has_featured_posts( $minimum = 1 ) {
	if ( is_paged() )
		return false;

	$minimum = absint( $minimum );
	$featured_posts = apply_filters( 'bubble_get_featured_posts', array() );
...
if ( ! function_exists( 'bubble_display_featured_posts' ) ) :
/**
 * Output the Jetpack Featured Content.
 */
function bubble_display_featured_posts() {
	if ( is_home() && bubble_has_featured_posts() ) {
		get_template_part( 'template-parts/featured', 'slider' );
	}
}
endif;
add_action( 'bubble_content_before', 'bubble_display_featured_posts' );

if ( ! function_exists( 'bubble_display_icon' ) ) :
/**
 * Display theme icons.
 */
function bubble_display_icon( $type ) {
	switch ( $type ) {
		case 'add': ?>
...
if ( ! function_exists( 'bubble_strip_gallery' ) ) :
/**
 * Removes the first gallery on the page.
 */
function bubble_strip_gallery( $content ) {
...
if ( ! function_exists( 'bubble_get_gallery' ) ) :
/**
 * Get gallery from content for gallery format posts.
 */
function bubble_get_gallery() {
	$gallery = get_post_gallery( get_the_ID(), false );
	if ( ! empty( $gallery ) && ! has_action( 'wp_footer', 'bubble_enqueue_flexslider' ) ) {
		add_action( 'wp_footer', 'bubble_enqueue_flexslider' );
	}
...
if ( ! function_exists( 'bubble_get_image' ) ) :
/**
 * Gets the first image from the page.
 */
function bubble_get_image() {
...
if ( ! function_exists( 'bubble_strip_image' ) ) :
/**
 * Removes the first image on the page.
 */
function bubble_strip_image( $content ) {
	return preg_replace( '/<img[^>]+\>/i', '', $content, 1 );
}
endif;

if ( ! function_exists( 'bubble_get_video' ) ) :
/**
 * Get the video from the current post.
 */
function bubble_get_video() {
	$first_url    = '';
	$first_video  = '';
...
if ( ! function_exists( 'bubble_strip_video' ) ) :
/**
 * Removes the first video from the page.
 */
function bubble_strip_video( $content ) {
	if ( bubble_get_video() ) {
...
if ( ! function_exists( 'bubble_display_logo' ) ) :
/**
 * Display the logo or site title and site description.
 */
function bubble_display_logo() {
	$logo = get_theme_mod( 'logo' );

	if ( $logo ) {
		$attrs = apply_filters( 'bubble_logo_attributes', array( 'class' => 'custom-logo' ) );

		?><a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home">
			<span class="screen-reader-text"><?php esc_html_e( 'Home', 'bubble' ); ?></span><?php
...
if (
	...
	if ( ! function_exists( 'bubble_logo_lazy_load_exclude' ) ) :
		/**
		 * Exclude Logo from Lazy Load plugins.
		 */
		function bubble_logo_lazy_load_exclude( $attr, $attachment ) {
	...
	add_filter( 'wp_get_attachment_image_attributes', 'bubble_logo_lazy_load_exclude', 10, 2 );

	if ( ! function_exists( 'bubble_featured_image_lazy_load_exclude' ) ) :
		/**
		 * Exclude Featured Images from Lazy Load plugins.
		 */
		function bubble_featured_image_lazy_load_exclude( $attr, $attachment ) {
	...
	add_filter( 'wp_get_attachment_image_attributes', 'bubble_featured_image_lazy_load_exclude', 10, 2 );
endif;

if ( ! function_exists( 'bubble_read_more_link' ) ) :
/**
 * Filter the read more link.
 */
function bubble_read_more_link() {
...
add_filter( 'the_content_more_link', 'bubble_read_more_link' );
...
if ( ! function_exists( 'bubble_excerpt' ) ) :
/**
 * Outputs the excerpt.
 */
function bubble_excerpt() {
...
function bubble_post_meta() {
	...
	if ( comments_open() && get_theme_mod( 'post_comment_count', true ) ) { 
		...
		comments_popup_link( esc_html__( 'Leave a comment', 'bubble' ), esc_html__( 'One Comment', 'bubble' ), esc_html__( '% Comments', 'bubble' ) );
		echo '</span>';
	}
}
endif;

if ( ! function_exists( 'bubble_entry_thumbnail_meta' ) ) :
/**
 * Print HTML with meta information for the sticky status, current post-date/time, author, comment count and post categories.
 */
function bubble_entry_thumbnail_meta() {
	echo '<div class="thumbnail-meta">';
	if ( is_sticky() && is_home() && ! is_paged() ) {
		echo '<ul><li>' . esc_html__( 'Sticky', 'bubble' ) . '</li></ul>';
	}
	if ( has_category() ) {
...
if ( ! function_exists( 'bubble_is_post_loop_widget' ) ) :
/**
 * Checks if we're currently rendering a post loop widget
 */
function bubble_is_post_loop_widget() {
...
if ( ! function_exists( 'bubble_related_posts' ) ) :
/**
 * Display related posts on single posts.
 */
function bubble_related_posts( $post_id ) {
	if ( function_exists( 'related_posts' ) ) { // Check for YARPP plugin (https://wordpress.org/plugins/yet-another-related-posts-plugin/).
		...
		<div class="related-posts-section">
			<h2 class="related-posts"><?php esc_html_e( 'You May Also Like', 'bubble' ); ?></h2>
							...
							<li>
								<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
									...
									if ( has_post_thumbnail() && is_active_sidebar( 'sidebar-main' ) )
										the_post_thumbnail( 'bubble-247x164-crop' );
									elseif ( has_post_thumbnail() )
										the_post_thumbnail( 'bubble-354x234-crop' );
									?>
			...
			<?php else : ?>
				<p><?php esc_html_e( 'No related posts.', 'bubble' ); ?></p>
			<?php endif; ?>
		</div>
		<?php wp_reset_query();
	}
...
if ( ! function_exists( 'bubble_tag_cloud' ) ) :
/**
 * Filter the Tag Cloud widget.
 */
function bubble_tag_cloud( $args ) {
	$args['unit']     = 'px';
	$args['largest']  = 12;
	$args['smallest'] = 12;
	return $args;
...
add_filter( 'widget_tag_cloud_args', 'bubble_tag_cloud' );
