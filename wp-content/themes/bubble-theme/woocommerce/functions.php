...
function bubble_woocommerce_scripts() {
	...
	wp_enqueue_style( 'bubble-woocommerce-style', get_template_directory_uri() . '/woocommerce' . BUBBLE_THEME_CSS_PREFIX . '.css', array(), BUBBLE_THEME_VERSION );

	// WooCommerce JavaScript.
	wp_enqueue_script( 'bubble-woocommerce-script', get_template_directory_uri() . '/woocommerce/js/jquery.woocommerce'  . BUBBLE_THEME_JS_PREFIX . '.js', array( 'jquery', 'wc-add-to-cart-variation' ), BUBBLE_THEME_VERSION, true );
	...
	wp_localize_script( 'bubble-woocommerce-script', 'bubble_data', $script_data );
...
if ( ! function_exists( 'bubble_woocommerce_update_cart_count' ) ) :
/**
 * Update cart count with the masthead cart icon.
 */
function bubble_woocommerce_update_cart_count( $fragments ) {
	ob_start();
	?>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'bubble_woocommerce_update_cart_count' );

/**
 * Filter the product archive pagination.
 */
function bubble_woocommerce_pagination_args( $array ) {
	$array = array(
...
add_filter( 'woocommerce_pagination_args', 'bubble_woocommerce_pagination_args', 10, 1 );

/**
 * Change the gallery thumbnail image size.
 * @link https://github.com/woocommerce/woocommerce/wiki/Customizing-image-sizes-in-3.3-
 */
function bubble_woocommerce_single_gallery_thumbnail_size( $size ) {
	return array(
		'width'  => 150,
		'height' => 150,
		'crop'   => 1,
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'bubble_woocommerce_single_gallery_thumbnail_size' );
