 ...
 *
 * Handles the primary JavaScript functions for the theme - (c) Bubble, freely distributable under the terms of the GPL 2.0 license. 
 */

/* globals jQuery, bubble */

( function( $ ) {

	// Element viewport visibility.
	$.fn.bubbleIsVisible = function() {
	...
	var calcHeaderHeight = function() {
		...
		if ( $mh.data( 'scale-logo' ) && ! $mh.hasClass( 'stuck' ) ) {
			...
			headerHeight += mhPadding - mhPadding * bubble.logoScale * 2;
		}

		return headerHeight;
	};

	$.fn.bubbleSmoothScroll = function() {
	...
	$( window ).on( 'load', function() {
		$( '#site-navigation a[href*="#"]:not([href="#"]), .comments-link a[href*="#"]:not([href="#"]), .woocommerce-review-link[href*="#"]:not([href="#"]), .bubble-scroll[href*="#"]:not([href="#"])' ).bubbleSmoothScroll();
	...
	$( window ).on( 'scroll', bubbleSelected );
	...
	$( '#mobile-menu-button' ).on( 'click', function( e ) {
		...
		$( '#mobile-navigation a[href*="#"]:not([href="#"])' ).bubbleSmoothScroll();

	} );

	$( window ).on( 'load', function() {

		bubble.logoScale = parseFloat( bubble.logoScale );
		...
		if ( $mh.data( 'scale-logo' ) ) {
			...
			var $img = $mh.find( '.site-branding img' ),
				...
				scaledWidth = imgWidth * bubble.logoScale;
				scaledHeight = imgHeight * bubble.logoScale;
			...
			var smResizeLogo = function() {
				...
				if ( top > 0 ) {
					$mh.css( {
						'padding-top': mhPadding.top * bubble.logoScale,
						'padding-bottom': mhPadding.bottom * bubble.logoScale
					} );
				...
				if ( $img.length ) {
					...
					if ( top > 0 ) {
						...
						var logoScale = bubble.logoScale + ( Math.max( 0, 48 - top ) / 48 * ( 1 - bubble.logoScale ) );
						if ( $img.height() != scaledHeight || $img.width() != scaledWidth || logoScale != bubble.logoScale ) {
					...
					$branding.css( 'transform', 'scale(' + bubble.logoScale + ')' );
		...
		if ( $( '#masthead' ).hasClass( 'sticky' ) ) {
			...
			var smSetup = function() {
				...
				if ( $( 'body' ).hasClass( 'mobile-header-ns' ) && ( $( window ).width() < bubble.collapse ) ) {
					return;
				}
				...
				if ( ! $( 'body' ).hasClass( 'no-topbar' ) && ! $tb.bubbleIsVisible() ) {
					$( 'body' ).addClass( 'topbar-out' );
				}

				if ( $tb.length && $( 'body' ).hasClass( 'topbar-out' ) && $tb.bubbleIsVisible() ) {
